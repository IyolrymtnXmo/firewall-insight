...
Every honesty string survived, by design and by test: *"Physical cabling,
switches and live routing are not inferred"*, *"…which is not the same as
...
- `i.enabled?'good':(i.configured?'bad':'purple')` → the three-way decision
  holds and the third branch is not a verdict class.
...
Element ids, renderer function names, the `TOPO` contract and `buildTopoGraph()`
returning `role:'routed-network'` / `kind:'route'` were not touched.
